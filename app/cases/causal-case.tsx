...
const figma = 'https://www.figma.com/design/VjoMjcvVOToZtwj4Ekfd5f/Causal-Labs-Demo?node-id=2279-9702';
const chapters = [
...
export function CausalCase() {
  return <><CaseHeader/><main id="main" className="causal-case">
    ...
    <section className="causal-result"><span className="eyebrow">THE DESIGN OUTCOME</span><h2>A connected experience.<br/>A testable <i className="serif">design direction.</i></h2><p>The design brings geographic context, local consequences, and prediction timing into a continuous flow. The animated prototype makes those transitions tangible, from the initial overview through an airport investigation to model comparison.</p><p>This case presents the interface and interaction work. The detection advantage and disruption figures shown in the demo are scenario values, not validated performance claims.</p><div className="causal-links"><a className="pill-link" href="/causal-brain" target="_blank" rel="noreferrer">Open interactive prototype ↗</a><a className="text-link" href={figma} target="_blank" rel="noreferrer">View the Figma flow ↗</a></div></section>
